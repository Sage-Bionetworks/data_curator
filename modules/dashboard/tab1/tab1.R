...
selectedDataTypeTab <- function(id, userName, uploadData, reqData, template, tabId, validationTab, parent) {
  moduleServer(
    id,
    function(input, output, session) {
          ...
          column(12, 
            ...
            column(4, class = "stats-item",
              tagList(
                ...
                div(class = "stat-text", 
                  h4("Outdate", style = "border-bottom: 0.6px solid #ff9900;"), span(2)
                )
              )
            )
          ),
      ...
      dbCheckList("checklist", uploadData, reqData, config)
      # networks plot for requirements of selected template
      dbNetwork("network", uploadData, reqData, template)
