...
test_that("manifest_generate returns an xlsx", {
  ...
  tmp <- tempfile()
  writeBin(xlsx, tmp)
  expect_true(nrow(readxl::read_excel(tmp)) > 0)
...
test_that("manifest_validate passes and fails correctly", {
  ...
  pass <- manifest_validate(data_type="Biospecimen",
...
test_that("storage_project_datasets returns available datasets", {
  skip_it()
  spd <- storage_project_datasets(asset_view="syn23643253",
                           project_id="syn26251192",
                           input_token=Sys.getenv("SYNAPSE_PAT"))
  expect_equal(spd, list(list("syn26251193", "Issue522")))
  
})

test_that("storage_projects returns available projects", {
  skip_it()
  sp <- storage_projects(url=file.path(schem_url, "v1/storage/projects"),
                   asset_view="syn23643253",
                   input_token=Sys.getenv("SYNAPSE_PAT"))
  expect_equal(sp[[1]][[1]], "syn23643250")
})
...
test_that("storage_dataset_files returns files", {
  ...
  sdf <- storage_dataset_files(asset_view = "syn23643253",
                        ...
                        input_token=Sys.getenv("SYNAPSE_PAT"))
  expect_equal(sdf[[1]], list("syn25705259", "Boolean Test"))
})
